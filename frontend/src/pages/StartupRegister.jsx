...
export default function StartupRegister() {
  ...
  const handleNext = (e) => {
    ...
    if (step === 1) {
      ...
      if (!startupName) {
        setError('Введите название стартапа');
        return;
      }
      setLoading(true);
      ...
      const registerUser = async () => {
        try {
          ...
          const response = await fetch('http://localhost:5000/api/startup-users/register', {
            method: 'POST',
            headers: {
          ...
          localStorage.setItem('startupToken', loginData.token);
          ...
          // Редиректим на дашборд стартапа
          setTimeout(() => {
