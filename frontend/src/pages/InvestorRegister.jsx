...
export default function InvestorRegister() {
  ...
  const handleNext = (e) => {
    ...
    if (step === 1) {
      ...
      if (!investorType) {
        setError('Выберите тип инвестора');
        return;
      }
      setLoading(true);
      ...
      const registerUser = async () => {
        try {
          ...
          const response = await fetch('http://localhost:5000/api/investors/register', {
            method: 'POST',
            headers: {
          ...
          localStorage.setItem('investorToken', loginData.token);
          ...
          // Редиректим на дашборд инвестора
          setTimeout(() => {
